...
to setup
  ...
  ;; define scenario
  ifelse abundance > 500000 [set scenario "HA"][set scenario "LA"]
  ifelse clump-size > 500 [set scenario (word scenario "clump")][set scenario (word scenario "uni")]
  ifelse qual-mean > 10 [set scenario (word scenario "LP")][set scenario (word scenario "SP")]
  type scenario type behaviorspace-experiment-name print behaviorspace-run-number
...
to color-patches
  (ifelse
    (penergy < (qual-mean - qual-sd)) [set pcolor yellow]
    (penergy < (qual-mean + qual-sd)) [set pcolor green]
    (penergy > (qual-mean + qual-sd)) [set pcolor 74]
  )
end
...
to go ;; put main schedule procedures for readability
  ...
  if ticks mod (24 * 7) = 0 [if group-recog-module? [search_for_groups]]
...
to make_csv_group-level_data
  let folder-path "C:\\Users\\Marcy\\Desktop\\jan 5 2022 ch2 study2"

  set-current-directory folder-path
  let file-name (word scenario behaviorspace-run-number ".csv")
...
to make_csv_network_patchdist [groupaslist]
  ...
  ;;below was how we saved social network as a matrix
;  let groupmembers sort group-member-neighbors
;  ;; groupmembers is now an ordered list of agents, e.g., (primate 0) (primate 2) (primate 28)
;
;  file-type " , " ;; leave first cell blank for indentation
;
;  ;; write the column names as each primate in the group
...
;
;   foreach groupmembers [h ->
;     ;; what is the distance between i and h?
;     let this-distance 0
;      ask h [
;        set this-distance distance i
;      ]
;      file-type (word this-distance ", ")
;
;  ]
;  file-print " "  ;; end row
;  ]
...
<experiments>
  ...
  <experiment name="8" repetitions="5" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
    <timeLimit steps="4300"/>
    <metric>foraging-efficiency-time</metric>
